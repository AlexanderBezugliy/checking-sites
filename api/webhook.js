...
export default async function handler(req, res) {
    ...
    if (callback_query && callback_query.data === 'check_now') {
        ...
        try {
            // 1. Отвечаем Телеграму (тут встроенный fetch работает без сбоев)
            ...
            // 2. Стучимся в GitHub через классический модуль https (обход бага DNS на Vercel)
            const postData = JSON.stringify({ ref: 'main' }); // если у тебя ветка master, замени 'main' на 'master'
